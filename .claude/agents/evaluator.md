...
| 契約 | 宣言されている契約(docstring の「常に exit 0」等)が全経路で成立するか |

「契約」は、正常系だけでなく異常系(読めない入力・壊れた入力・出力先が閉じられた場合・
外部コマンドのハング)でも実際に実行して確認する。宣言と実挙動が食い違えば
NEEDS_REVISION の根拠にする。
...
4. 計画に書かれた評価コマンドを実際に実行する
   評価コマンドの実行時は、出力を必ず logs/runs/ に tee で保存する
   (generator と同じ形式。`set -o pipefail` を先頭に付け、元コマンドの
   失敗が tee の成功で隠れないようにする)。レポートに書く数値は、
   このログファイルから引用できる状態にする。

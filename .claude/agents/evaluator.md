...
4.5. ベースライン比較実行(手順4の検証コマンドが1つでも失敗した場合のみ実行する。
   PASS 時は実行しない)
   a. 分岐元を決める(タスク指示に親ブランチの指定があればそれを使う。無ければ
      `git merge-base HEAD main` で求める)。
   b. `git worktree add --detach <作業スコープ>/.worktrees/baseline <分岐元>` で
      ベースライン用の worktree を作る(作業スコープ配下に作る理由は guard_scope。
      ml-pipeline.md の手順5で generator の worktree を作業スコープ配下に作らせている
      規約と同じ)。
   c. その中で手順4と同じ検証コマンドを実行し、出力を logs/runs/ に tee で保存する。
   d. 確認後、`git worktree remove` で必ず後片付けする。

   判定への反映:
   - ベースラインでも同じテストが落ちる → 今回の変更が原因ではない。
...
| テスト名 | ブランチ結果 | ベースライン結果 | 判定への採否 |
|---|---|---|---|
| <検証コマンド> | PASS/FAIL | PASS/FAIL/flaky/実施できず | 根拠にする/既存の失敗として除外/flakyのため除外 |

...
4.5. ベースライン比較実行(手順4の評価コマンドが1つでも失敗した場合のみ実行する。
   PASS 時は実行しない)
   1. 分岐元を決める(タスク指示に親ブランチの指定があればそれを使う。無ければ
      `git merge-base HEAD main` で求める)。
   2. `git worktree add --detach <作業スコープ>/.worktrees/baseline <分岐元>` で
      ベースライン用の worktree を作る(作業スコープ配下に作る理由は guard_scope。
      ml-pipeline.md の手順5で generator の worktree を作業スコープ配下に作らせている
      規約と同じ)。
   3. その中で手順4と同じ評価コマンドを実行し、出力を logs/runs/ に tee で保存する。
   4. 確認後、`git worktree remove` で必ず後片付けする。

   判定への反映:
   - ベースラインでも同じ評価コマンドが失敗する → 今回の変更が原因ではない。
     NEEDS_REVISION の根拠にせず「既存の失敗」としてレポートに列挙する。
   - ブランチでのみ落ちる → 従来どおり NEEDS_REVISION の根拠にする。
   - ブランチ側で2回実行して結果が一致しない → flaky として記録し根拠にしない。

   worktree が作れない場合(権限確認で拒否・分岐元が特定できない・git worktree が
   使えない等)は比較をスキップし、レポートに「ベースライン比較: 実施できず(理由)」と
   明記する(黙って省略しない)。
...
#### ベースライン比較(手順4.5を実行した場合のみ)
| 評価コマンド | ブランチ結果 | ベースライン結果 | 判定への採否 |
|---|---|---|---|
| <評価コマンド> | PASS/FAIL | PASS/FAIL/flaky/実施できず | 根拠にする/既存の失敗として除外/flakyのため除外 |
...
- cross-review(Codex)のレポートが渡されている場合: Codex の指摘と
  自分の指摘が**同じ箇所**(同一ファイルの同一関数・同一行付近)で
  **同種の問題**を指していたら、その指摘の重大度を1段引き上げる
  (同じ関数への別種の指摘は一致と数えない。ml-pipeline「多モデル合議の規約」)

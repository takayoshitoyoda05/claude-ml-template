...
| R-013 | PASS | `diff <(grep -oE '"CLAUDE_[A-Z_]+\|' claude-init.sh \| tr -d '"\|' \| sort -u) <(grep -oE '"CLAUDE_[A-Z_]+"' claude-init.ps1 \| tr -d '"' \| sort -u)` | exit 0、両方とも生・一意ともに11件で一致 | claude-init.sh:136(`CLAUDE_SESSION_MONITOR`); claude-init.ps1:128(`CLAUDE_SESSION_MONITOR`) |

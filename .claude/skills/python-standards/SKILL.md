...
- 手段2(依存を増やせない場合の代替): docstring に Shape 節(入力・出力の
  次元)を書き、テストで `assert x.shape == (...)` を1つ以上置く。

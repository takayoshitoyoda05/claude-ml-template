...
10. /ml-pipeline の spec-checklist ゲート(手順3.3)から NEEDS_WORK の指摘を
    受けて呼ばれた場合は、指摘された箇所だけを対象に質問し、解消する
    (全カテゴリの再インタビューはしない)。
    - 設計書がある場合: 設計書を更新して ml-pipeline に制御を返す。
    - 設計書が無い場合: 解消した回答を ml-pipeline に返し、
      planner の計画修正に使わせる。
    単独で呼ばれた場合は、完了時に「/ml-pipeline に渡すと
...
## 完了時
11. 固まった設計書を docs/active/ に移動する準備が整ったことをユーザーに

---
name: config-explain
description: 現在有効なスコープ・評価強制・コミット規約の設定と、その設定がどこ(settings.local.json かシェル環境変数か)から来ているかを一覧表示する。「今の設定を確認して」「なぜブロックされたか分からない」と言われたときに使う。
---
...
## 進め方
1. 作業スコープ配下の .claude/settings.local.json を読む(無ければ「未設定」とする)。

...
| 次元 | 検査内容 | 例 |
|------|---------|-----|
| 完全性 | 曖昧性タクソノミーの全カテゴリ(境界値・同一性・解決・例外系・状態・順序・データ・非機能・スコープ境界・既存契約。研究系なら仮説・変数・ベースライン・成功基準・再現性も)に答えが書かれているか。同一性・解決に該当する要件が操作形でなく不変条件として書かれ、同値クラス表が添付されているか | 「NaN 入力時の挙動が書かれていない」「パス照合の要件が『cwd 基準に変更する』という操作形のままで、symlink 等の同値クラスが未定義」 |
...
2. 対象の設計書(無ければ Planner の計画)を読み込む。

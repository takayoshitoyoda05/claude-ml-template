---
name: branch-naming
description: 新しいブランチを作る前に、プロジェクトのブランチ命名規則を探して名前を決める。ml-pipeline の手順1.5 で自動で使われる。単独でも「ブランチ名を決めて」「命名規則を調べて」と言われたときに使う。
---
...
docs/ はこのプロジェクトの .gitignore 対象(git 管理外のローカルキャッシュ)
のため、この保存はコミットされない。

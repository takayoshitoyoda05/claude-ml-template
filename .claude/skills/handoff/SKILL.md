---
name: handoff
description: 別セッション・別マシンで作業を再開するために区切るとき、「引き継ぎを作って」「handoffして」「ここで一旦区切りたい」と言われたときに必ず使う。
---
...
## 実行状態スナップショット
`.claude/state/<slug>.json`(存在する場合)の内容をそのまま埋め込むこと(必須)。
散文の要約だけでなく、state_gate がスキーマ検証済みの構造化状態も残す。

- 埋め込み方: 該当ファイルの中身をコードブロックでそのまま貼る
- 別マシンでの復元: 受け取った側は同じ内容を `.claude/state/<slug>.json` に
  書き戻してから作業を再開する(state_gate が書き戻し時にスキーマを検証する)
...
`.claude/state/<slug>.json` も同様に gitignore 対象でマシンをまたがないため、
別マシンへ引き継ぐ場合は上記「実行状態スナップショット」節での埋め込みが必要になる。

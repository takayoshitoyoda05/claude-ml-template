...
## 未確定事項(ユーザーの回答が必要。回答前に Step 8・Step 9 の該当部分を実装しない)

推測で埋めないため質問リストとして残す。Q1・Q2 は R-014 の配線先に、Q3 は
`data_unlock.py` の引数仕様に直結する。**Q1〜Q3 以外の設計判断はすべて本計画で確定済み。**

| Q | 内容 | 選択肢と影響 |
|---|------|-------------|
| Q1 | `claude-init` の OPTIONAL_FEATURES に `CLAUDE_DATA_PROFILE` をどう載せるか。**確認済みの制約**: `enable_feature()`(`claude-init.sh:144-154`)は値を `"1"` 固定で書き換えるため、3値(public/internal/sensitive)を取るプロファイルをそのまま載せると `CLAUDE_DATA_PROFILE=1` という無効値になる | **A(推奨)**: OPTIONAL_FEATURES には 1/0 フラグの `CLAUDE_DATA_NO_READ`(+Q2次第で `CLAUDE_DATA_GATE`)だけを載せ、`CLAUDE_DATA_PROFILE` は settings.local.json.template と config-set / config-explain だけで扱う。installer の共通ロジックを触らないため回帰リスクが最小 / **B**: `enable_feature` を「変数名\|説明\|設定値」形式に拡張して3値を対話選択できるようにする。installer 4本の共通ロジック変更となり、既存11機能すべての回帰確認が必要 / **C**: A+プロファイル専用の追加プロンプトを別途書く。対話コードが二系統になる |
| Q2 | `CLAUDE_DATA_GATE`(Phase 2 で追加)は現状 config 系にまったく配線されていない(**確認済み**: `grep -rn CLAUDE_DATA_GATE .claude/commands/ templates/ .claude/skills/` はヒット0。README のみ)。Phase 3 の R-014「CLAUDE_DATA_PROFILE **等**」にこれを含めるか | **A(推奨)**: PROFILE / NO_READ / GATE の3変数をまとめて配線する。プロファイル解決の説明は GATE 抜きには書けない(config-explain が「なぜ遮断されたか」を説明できない)ため / **B**: PROFILE と NO_READ のみ配線し、GATE は Phase 2 の欠落として別途扱う。今回の diff は小さいが、config-explain の説明が不完全なまま残る |
| Q3 | `data_unlock.py --minutes` の既定値と上限 | **A**: 既定30分・上限なし(設計書の例に忠実。長時間の解除を止める仕組みが無い) / **B(推奨)**: 既定30分・上限240分(超過時はエラーで非0終了)。解除しっぱなしの事故を構造的に防ぐ / **C**: 上限を環境変数で可変にする(設定項目が増え、解除の抜け道にもなる) |
...
| ファイル | 変更内容 |
|---------|---------|
| `tests/test_data_protection_phase3.py` | 新規。PC-1〜PC-24 の受け入れテスト |
...
| `.claude/hooks/data_unlock.py` | 新規(staging が配置)。ユーザー `!` 実行専用の一時解除 |
...
| `templates/settings.local.json.template` | プロファイル系の項目追加 |
| `.claude/skills/config-set/SKILL.md` / `config-explain/SKILL.md` | 変数表に追加 |
| `claude-init.sh` / `claude-init.ps1` / `claude-update.sh` / `claude-update.ps1` | 新 scripts 2本を配布リスト + IGNORE_ENTRIES に、OPTIONAL_FEATURES に該当フラグ |

## 事後条件(postconditions)

期待値は設計書の受け入れ条件から導いた。実装前に Step 1 でテスト化する。
...
| PC-15 | `templates/settings.local.json.template`・`config-set/SKILL.md`・`config-explain/SKILL.md`・`claude-init.sh`/`.ps1` | ファイル内容 | Q1・Q2 の回答で確定した変数名がすべてに現れる。template は `json.loads` でパースでき、既存キーが1つも消えていない | R-014 |
...
| # | 内容 | 対象ファイル | 依存 | 並列グループ |
|---|------|-------------|------|-------------|
| 1 | **テストファースト**: PC-1〜PC-24 を受け入れテストとして書き、全件 RED(または skip)を確認する。`tests/test_data_protection_phase2.py` の様式に倣う(冒頭 docstring で契約明記・`_ROOT` 定数・subprocess 起動・`pytestmark_staging` 相当の skip・doctor は同ファイル 252-295 行の `place_installers` 方式)。テスト名は設計書の `-k` キーワード(`read_gate_blocks` 等)にそのまま一致させる。**ここで固定する契約**: (a) 解除記録 `.claude/spec/data_unlock.txt` の形式 = UTC epoch 秒の整数1行、(b) `CLAUDE_DATA_NO_READ` の値の形 = `1` または data/ 直下のサブディレクトリ名のカンマ区切り、(c) プロファイル解決の戻り値 = (NO_READ 有効, GATE 有効) の2値。Step 2〜9 が別グループで並列実装されるため、ここで固定しないと食い違う。staging 依存のケース(PC-1〜PC-10・PC-13・PC-14・PC-22)は `_staging_data_protection_p3.py` または `.claude/hooks/data_read_gate.py` が無ければ skip する。**注意**: age 不在経路のテストで PATH を空にすると python 自体が起動できなくなるので、`sys.executable` の絶対パスで起動しつつ PATH だけを空ディレクトリに差し替える (R-001〜R-023 対応) | `tests/test_data_protection_phase3.py` | なし | A |
...
| 7 | README を追記する。3.21 節に (a) data/synthetic の役割詳細(実データと同スキーマの合成サンプル・遮断対象外・テストとデバッグはここで行う)、(b) 読み取り遮断とプロファイルの説明、(c) 一時解除の `!` 実行手順、(d) age 復号の手動手順と秘密鍵は環境外管理である旨、(e) Grep/Glob は遮断対象外という既知の限界。あわせて環境変数表(263行付近)・4.5 節の doctor マーカー一覧・ファイルツリーの scripts/ 一覧に新規分を追記する。既存 3.21 節の小見出し(`#### バックアップ記録(data/.backup_stamp)` 等)の粒度に倣う (R-018 対応) | `README.md` | Step 1 | E |
| 8 | 設定の配線(**Q1・Q2 の回答後に着手**)。`templates/settings.local.json.template` に変数を追加し、`config-set` の雛形 JSON と変数表、`config-explain` の変数表に同じ変数を追加する。既存の `CLAUDE_SESSION_MONITOR` の並び・説明文の書式に倣う。template は JSON として妥当なまま保つ (R-014 対応) | `templates/settings.local.json.template`, `.claude/skills/config-set/SKILL.md`, `.claude/skills/config-explain/SKILL.md` | Step 1, Q1・Q2 の回答 | E |
| 9 | 配布の配線。`claude-init.sh` / `claude-init.ps1` / `claude-update.sh` / `claude-update.ps1` の scripts 配布リストに `data_summary.py` と `backup_encrypt.py` を追加し、`claude-init.sh:107-109` と `claude-update.sh:92` 相当の IGNORE_ENTRIES にも追加する(sh/ps1 で1対1)。あわせて OPTIONAL_FEATURES に Q1・Q2 の回答で決めたフラグを追加する。**注意**: doctor の scripts 差分検査は `find` による全走査なので追加変更は不要(現状分析の裏取り済み)。既存 `scripts/data_scan.py` の記載箇所と同じ並びに足す (R-014, R-019 対応) | `claude-init.sh`, `claude-init.ps1`, `claude-update.sh`, `claude-update.ps1` | Step 1, Q1・Q2 の回答 | F |
...
| R-008 | Step 1, 4 | 同 `-k unlock_window` |

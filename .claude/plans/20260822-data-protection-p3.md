...
| PC-6 | `.claude/hooks/data_gate.py` | env `CLAUDE_DATA_NO_READ=1`、cmd `cat data/raw/x.csv` / `head -5 data/raw/x.csv` / `tail data/raw/x.csv` / `less data/raw/x.csv` / python のワンライナーで同ファイルを開くコマンド | すべて exit 2。同じ cmd を env `CLAUDE_DATA_GATE=1` のみ(NO_READ・PROFILE 空)で与えると exit 0(Phase 2 の挙動不変)。**追加(premortem 2周目 MEDIUM の反映)**: クォート内にシェルメタ文字を含む読みコマンド(例: 正規表現 `a\|b` を引数に持つ grep で data/ 以外を読むもの)が誤ブロックされる場合は既存 `_SEGMENT_SPLIT` の既知の限界として**挙動をテストで固定**し(誤検知は fail-closed 側)、README の既知の限界に1行明記する | R-006 |
...
| PC-9 | `.claude/hooks/guard_bash.py` | cmd `uv run python .claude/hooks/data_unlock.py --minutes 30` / `cp .claude/hooks/data_unlock.py /tmp/x.py` / `grep -n minutes .claude/hooks/data_unlock.py` | 前2つは exit 2(stderr に `!` 実行の案内)、grep は exit 0。**追加**: クォート内 `\|` を含む grep(data_unlock を参照)も既存 spec_approve ブロックと同一挙動であることをテストで固定(既知の限界の継承。premortem 2周目 MEDIUM) | R-009 |

...
### Standards軸レビュー対応(2軸レビュー後の修正): 完了
Spec軸(evaluator)はR-101〜R-112全PASS、Standards軸の指摘4件+README不整合1件を修正。
- `spec_staging/_common.py` に `resolve_docs_dir` / `resolve_spec_dir` を一元化し、
  `spec_gate.py` / `spec_approve.py` 両方の重複実装を撤去してimportに変更。
- 両ファイルの `sys.path.insert(0, ...)` + `# noqa: E402` を撤去し、既存
  `guard_scope.py` / `guard_bash.py` と同じ「素の `from _common import ...`」に統一。
- `spec_gate.py` の `spec_dir_signature` / `docs_dir_signature` の重複を
  `_files_signature(paths)` ヘルパーに統合。
- `check_auto_recheck` の print責務をmain側に移し、他の `check_*` と同様に
  `(reasons, ログ文字列)` を返す純粋関数に統一(R-108のログ内容は不変)。
- `README.md` のフック一覧テーブルの `spec_gate.py` / `spec_approve.py` 行に
  「要手動配置」注記を追加し、ファイル一覧テーブルと表記を統一。
- 再検証: `spec_staging/hooks/` に `_common.py` を一時配置したフィクスチャで
  R-101(exit0)・R-102(exit2)・R-103(exit2)・R-104(exit2)・R-105(exit0)・
  R-107(exit2)・R-108(ログにR-001,R-003出現)・R-112(exit0)が全て期待通り。
  R-106は`.claude/hooks/_common.py`未適用のため引き続きexit0(フェーズ5待ちの
  想定通りで回帰ではない)。`bash verify-hooks.sh` の既存23ケースもPASSを再確認。

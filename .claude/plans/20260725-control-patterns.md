...
- 設計書: `docs/drafts/control-patterns-spec.md`(docs/active/ へは移動しない。理由は現状分析を参照)
...
| ファイル | 区分 | 変更内容 | 設計書 |
|---|---|---|---|
| .claude/improvements/invariants.md | MOD | 末尾に「## リソース上限(resources)」と「## 人間の承認が必須の操作(HITL)」を追記 | S1 |
...
| .claude/agents/evaluator.md | MOD | 「## goal との突き合わせ(実験計画の場合)」節を追記 | S4 |
...
| A-3 | plan_gate.py を設計書 L162-258 のコードそのままで新規作成(S3 対応)。**保護パスのため generator は書き込めない**(guard_scope が exit 2)。スクラッチに全文を用意し、**ユーザーが手動適用**する。適用後 `git add .claude/hooks/plan_gate.py`(明示パス指定の add は guard_bash を通る)。UTF-8(BOMなし)/LF/実行属性不要 | .claude/hooks/plan_gate.py | A-1 | A |
| A-4 | Stop フック配列(L62-87)の末尾、notify.py の直後に `uv run python .claude/hooks/plan_gate.py` の command オブジェクトを追記(S3 対応)。**保護パスのためユーザーが手動適用**。適用後 `python -c "import json; json.load(open('.claude/settings.json'))"` で JSON 妥当性を確認 | .claude/settings.json | A-3 | A |
| A-5 | `./verify-hooks.sh` を実行し全PASS を確認(S9・S11 対応)。加えて検証方法節の「plan_gate ブロック挙動の一時ディレクトリ検証」を実行する | (実行のみ) | A-1〜A-4, B-1 | A |
| B-1 | 末尾(L33 の直後)に「## リソース上限(resources)」と「## 人間の承認が必須の操作(HITL)」を設計書 L85-109 のとおり追記(S1 対応)。resources は yaml コードブロックで書く(plan_gate が `max_train_minutes` 等を正規表現で読むため、キー名と数値の書式を変えない) | .claude/improvements/invariants.md | なし | B |
| B-2 | 「## 計画フォーマット」表(L54-62)の末尾に cost_estimate / goal の2行を追加し、「## 計画の実装手順の出力例」節(L90-112)の末尾に設計書 L129-149 の YAML 例と `experiment: false` の説明を追記(S2 対応)。設計書側の外側 ```markdown フェンスは引用のための囲いであり、本文には含めない | .claude/agents/planner.md | なし | B |
| B-3 | 「## goal との突き合わせ(実験計画の場合)」節を設計書 L276-292 のとおり追記(S4 対応)。挿入位置は「## 評価レポート形式」(L41-44)の直後・「## verdict ファイルの出力」(L46)の直前(判定に関する記述をまとめるため)。**既存の PASS/NEEDS_REVISION/FAIL 表記と verdict の PASS/FAIL/UNVERIFIABLE は変更しない**(設計書の指示は「追記」であり、既存フロー(ml-pipeline 手順7 の分岐)が依存しているため) | .claude/agents/evaluator.md | なし | B |
| B-4 | router エージェントを設計書 L301-327 のとおり新規作成(S5 対応)。frontmatter は `name: router` / `tools: Read, Grep, Glob, Bash` / `model: haiku` | .claude/agents/router.md | なし | B |
| C-1 | 「## 手順」(L26)の直前に「## 自律度レベル(CLAUDE_CONTROL_LEVEL)」節(設計書 L359-375)を挿入し、「### 1. 作業スコープの確定」(L27)の直前に「### 0. ルーティング(規模判定)」(設計書 L337-351)を挿入(S6 変更1・2 対応)。**自律度レベル表の「(手順4のステップ承認)」は「(手順5のステップ承認)」に直す**(現行の手順番号に合わせる) | .claude/commands/ml-pipeline.md | なし | C |
| C-2 | 「### 5. 実装」(L130)の冒頭に HITL 実行前承認(設計書 L418-425)、同節の本文末尾(手順5.5 の直前)にステップ承認(設計書 L383-388)、「### 7. 結果の集約」(L373)の本文末尾(手順7.5 の直前)に失敗遷移表(設計書 L396-410)を追記(S6 変更3・4・5 対応)。設計書が言う「手順4」=現行の手順5、「手順6の直後/### 6. 結果の集約」=現行の手順7 | .claude/commands/ml-pipeline.md | C-1 | C |
...
| D-1 | 3.1 エージェント表(L649-659)に router 行、3.4 フック表(L718-739)に plan_gate.py 行、環境変数テーブル(L193-209)に CLAUDE_CONTROL_LEVEL 行を、設計書 L502 / L508 / L514 のとおり追加(S10 対応) | README.md | なし | D |
| D-2 | 3.19 節(L1084-1130)の直後・「## 4. テンプレートの育て方」(L1134)の直前に「### 3.20 自律度レベルと人間のコントロール」を新設し、設計書 L520-562 の本文(手段の階層表 / 自律度レベル表 / goal / 失敗遷移表 / ルーティング)を入れる(S10 対応)。既存の節番号付き見出し規約に合わせて `3.20` を付ける。data/ 以外のデータディレクトリ名の場合に settings.local.json で追加できる旨(設計書 L461-462)も本節に1行入れる(S7 の README 記載要求) | README.md | D-1 | D |
...
- **判定語彙の重複**: 設計書が追加する三値(pass / fail / inconclusive)は、
  evaluator の既存レポート判定(PASS / NEEDS_REVISION / FAIL)および verdict の
  (PASS / FAIL / UNVERIFIABLE)と併存する。設計書の指示は「追記」であり、
  既存表記を置換すると ml-pipeline 手順7 の分岐と spec_gate の verdict 検査が
  壊れるため、**追記のみに留める**。用語の対応付けは未確定事項に記載。
...
|---|---|---|---|
| S1 | invariants に resources / HITL | B-1 | `grep -l "max_train_minutes" .claude/improvements/invariants.md` |
...
| S4 | evaluator の三値判定 | B-3 | `grep -l "inconclusive" .claude/agents/evaluator.md` |
| S5 | router エージェント | B-4 | `test -f .claude/agents/router.md` / `grep -l "model: haiku" .claude/agents/router.md` |
| S6 | ml-pipeline の手順0・自律度・ステップ承認・失敗遷移表・HITL | C-1, C-2 | `grep -l "ルーティング" / "CLAUDE_CONTROL_LEVEL" / "失敗遷移表" .claude/commands/ml-pipeline.md` |
...
- **仕様書ファイル(`docs/drafts/control-patterns-spec.md`)の削除は計画に含めない**。
  理由: 設計書 L15-16 が削除の前提を「verify-hooks 全PASS **かつ git push まで完了**」と
  定めており、push がユーザー承認事項である以上、削除もその後のユーザー操作になるため。
  パイプライン完了・マージ後に
  `git rm docs/drafts/control-patterns-spec.md` → コミット → push をユーザーが実行する。

## 未確定事項(回答があれば計画に反映する。無ければ上記の既定の扱いで進行可能)

1. **evaluator の判定語彙の対応付け**: 追加する三値(pass / fail / inconclusive)と、
   既存のレポート判定(PASS / NEEDS_REVISION / FAIL)・verdict(PASS / FAIL / UNVERIFIABLE)の
   関係を設計書は定義していない。
   - A(既定・本計画の扱い): 追記のみ。既存表記は一切変更しない(最小diff、既存フロー無影響)
   - B: goal の三値を既存レポート判定にマッピングする1文を足す(例: inconclusive → NEEDS_REVISION)
   - C: 既存表記を三値に統一する(ml-pipeline 手順7・spec_gate の verdict 検査への波及調査が必要)
2. **`data/` 以外のデータディレクトリ名**: 設計書 L461-462 は「README に記載する」と
...
- (b) ADR: 本計画のトレードオフ判断(deny リストの導入、自律度レベルの一括スイッチ)は
  いずれも設計書側で確定済みで、README 3.20 節に設計意図が記録される。計画側で新たに
  行った判断(drafts 据え置き・保護ファイルのユーザー手動適用・一時ディレクトリ方式)は
  いずれも既存前例の踏襲であり、新規 ADR は作成しない。

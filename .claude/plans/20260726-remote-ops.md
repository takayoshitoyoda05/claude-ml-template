...
| D-10 | 起動スクリプトは毎回「初回のみ必要な設定」の全文を表示する(設計書 S2/S3) | マシン単位のマーカーファイルが無いときだけ全文を表示し、表示後にマーカーを作る。2回目以降は上表の1行ヒントに縮める | ユーザーの追加要望(実装後)。トグルの状態は検出不能なため「案内を出したか」を自前で記録する。実測: `~/.claude.json` のトップレベル56キーを `enable\|toggle\|setting\|config\|session\|control` で検索しても該当なし、関連キーは `remoteControlUpsellSeenCount`(表示回数)と `tipLifetimeShownCounts.remote-control` のみで、有効/無効を示すキーは無い。マーカーの置き場所は sh=`$XDG_STATE_HOME`(無ければ `$HOME/.local/state`)配下の `claude-remote/notice-shown`、ps1=`$env:LOCALAPPDATA\claude-remote\notice-shown`(`~/.claude/` は Claude Code 本体の管理領域なので避ける)。**マーカーの作成失敗・パス計算失敗で起動を妨げないこと**。sh は `set -u` 下なので状態ディレクトリを `if [ -n "${XDG_STATE_HOME:-}" ] / elif [ -n "${HOME:-}" ] / else 空` の3分岐で決め、決定できなければマーカーに一切触れない(空文字のまま `[ -n ]` で弾く)。ps1 は `Join-Path` と `New-Item` を try/catch で保護し、`$NoticeMarker` が `$null` なら同様に触れない。**どちらも「決定できないときはルート直下等に書きにいかない」ことが条件**(root 実行で `/.local/` が生えるのを防ぐ) |
...
#       HOME と XDG_STATE_HOME の両方を消しても exit 0 で、bash -x のトレースに mkdir/touch が
#       現れないこと(状態ディレクトリを決定できないときはマーカーに触れない)。
#       XDG_STATE_HOME が空文字のときは $HOME/.local/state にフォールバックすること
...
- **既知の制約(修正せず記録): バージョン比較が極端な値で sh / ps1 と食い違う**。
  `doctor.sh` は POSIX awk の数値比較なので上限が無いが、`doctor.ps1` の
  `[version]::TryParse` は各要素が Int32(2147483647)を超えると失敗し
  「情報: バージョンを取得できませんでした」に倒れる。例: `2147483648.0.0` は
  sh が「対応」、ps1 が「取得できませんでした」になる(Codex クロスレビュー MEDIUM)。
  `claude` の実バージョンは 2.1.x であり、要素が21億を超えることは想定されないため
  修正しない。揃えるなら awk 側にも上限判定を足す。

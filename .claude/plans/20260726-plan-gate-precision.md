...
チェック対象外であることを行頭で宣言する。箇条書きにすると正規表現
...
6. `^\s*experiment\s*:\s*false` があれば exit 0。
...
- invariants.md が存在しない / `resources:` ブロックが無い / 特定の `max_*` キーが無い場合は、
  **その次元の上限比較のみをスキップ**する(C9 は「キーはあるのに読めない」場合だけ)。
  計画側の C1〜C8 は invariants の有無に関わらず実施する。
...
| R-003 | invariants の `max_*` が読めない場合ブロックし、キー / ブロック不在の次元は比較しない | 1, 2, 5, 6 | 同上(T-14〜T-16) | auto |

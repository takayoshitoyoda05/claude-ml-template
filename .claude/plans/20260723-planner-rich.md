...
- 確認済み: claude-init.sh / claude-update.sh は `.claude/{agents,commands,hooks,skills,output-styles,rules}` を
  ...
  → 新規 `templates/design-doc.md.template` は ADR/CONTEXT と同じ参照専用扱いとし、
     init/update の変更は不要(セクション5は「確認のみ・変更不要」で確定)。.ps1 は本機で構文検証不能のため触らない。
...
| `claude-init.sh/.ps1`, `claude-update.sh/.ps1` | 確認のみ | 変更不要(参照専用テンプレ前例に従う) | - |
...
| 8 | init/update の templates 配布を確認(変更なしを記録) | (確認のみ) | なし | - |
| 9 | 検証コマンド群+`bash ./verify-hooks.sh` を全実行 | (検証) | 1-7 | - |
...
- 冒頭ワークフロー mermaid(L7-30)を最小修正: `B --> C` を
  `B --> SC{spec-checklist<br>品質ゲート 手順3.3}` / `SC -->|READY| C` / `SC -->|NEEDS_WORK 最大2周| B` に置換。
...
- 日付見出しは既に 2026-07-22 → 2026-07-23 の昇順で正しい。
- コミット: `feat(step 7): CHANGELOG に設計書リッチ化を記録`

### Step 8 詳細(init/update 確認)
- `grep -n "templates" claude-init.sh claude-update.sh` で個別コピーのみ(丸ごと配布なし)を再確認。
- ADR/CONTEXT テンプレ同様の参照専用のため **変更なし**。.ps1 は触らない。コミットなし。
...
! grep -q "手順2\.3" .claude/skills/spec-checklist/SKILL.md   # 読み替え漏れが無いこと
...
feedback.md 追記の参照整合、および design-interview の番号連番性(4挿入・9受け入れ条件・「手順9の直後」見出し)を確認。

## コミット分割方針
`feat(step N)` 形式で Step 1〜7 を各1コミット(Step 8 は変更なしでコミットなし、Step 9 は検証のみ)。
...
- **検討した代替案2(init/update に templates 丸ごと配布を追加)**: ADR/CONTEXT テンプレが未配布の前例に反し、
  .ps1 変更(本機で構文検証不能)を伴うため不採用。参照専用扱いで一貫させる。

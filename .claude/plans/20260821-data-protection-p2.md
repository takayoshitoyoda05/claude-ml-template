...
| PC-19 | `scripts/precommit_data_check.py` | クリーンな小さいテキスト差分をステージ | exit 0 かつ実測経過時間 < 1.0 秒(3回実行の最小値で判定。共有マシン負荷によるフレーキー化を防ぐ。premortem LOW の反映) | R-019 |
...
| PC-28 | `claude-init.sh` / `.ps1` / `claude-update.sh` / `.ps1` / `doctor.sh` / `.ps1` | (a) **E2E**: 全変更をコミットした状態で、verify-installers.sh の place_installers 方式(TEMPLATE_REPO を `file://<ルート>` に sed 差し替え)のサンドボックスで claude-init.sh を実行 (b) sh/ps1 本文 (c) doctor E2E: サンドボックスの scripts/ 配下1ファイルを改変して doctor.sh を実行 | (a) サンドボックスに Phase 2 の scripts/ 8ファイル+env_fingerprint.py が**実在**する(存在チェック。静的 grep では配布バグを検出できないため実行で確認。premortem 2周目 HIGH の反映) (b) sh から抽出した配布ファイル名の一意集合と ps1 側が完全一致 (c) doctor 出力に scripts/ の `DIFF:` 行が現れる(差分検査が no-op でないことの実効確認) | R-028 |
...
| 8 | doctor に (i) データ保護節の4マーカー追加(`[DATA-LOCK-MISMATCH]` / `[DATA-BACKUP-STALE]` / `[DATA-BACKUP-UNKNOWN]` / `[DATA-PRECOMMIT-OFF]`。判定材料は `data/data.lock`・`data/.backup_stamp`・`git config core.hooksPath`)、(ii) 差分検査への scripts/ 追加 — **既存の `for item in ...` ループ(`.claude/$item` 前提)には足さない**。そのまま `scripts` を足すと `remote_dir="$TMP/.claude/scripts"` が不在で `continue` により無言の no-op になる。`doctor.sh:44` 以降の `agents/shared/` 用ブロック(リポジトリ直下パス専用)に倣った**別ブロック**として実装する(premortem 2周目 MEDIUM の反映)、を行う。**マーカーは既存3種と同じ `if [ -d "data" ]` / `if (Test-Path "data")` の内側**に、同じ `警告: [MARKER] 本文` 書式で置く(外側に置くと Phase 1 の `test_doctor_no_data_dir` の意図から外れる)。**終了コードは変えない**。sh と ps1 を必ず同時に編集する(R-004/R-005/R-020/R-021/R-028 対応) | `doctor.sh`, `doctor.ps1` | Step 1 | D |

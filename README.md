...
| guard_bash.py | PreToolUse (Bash/PowerShell) | 危険コマンド(`rm -rf /` の表記ゆれ、強制push等)、一括ステージ(`git add .` / `-A`)、秘密情報の `git add`、フック/設定/承認記録を変更するコマンド(`cp`/`mv`/`sed -i`/`rm`/リダイレクト/tee 等)、コミット規約(フラグON時)をブロック |

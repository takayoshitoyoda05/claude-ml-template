...
| CLAUDE_SECURITY_SCAN | `1` でclaude-securityプラグインによる差分スキャンを2軸レビュー後に実行 | 無効(0) |
...
| claude-security | オンデマンドのマルチエージェント脆弱性スキャン(6フェーズ、Panel 3視点検証で偽陽性除外済み) | 有料プラン必須。トークン消費大。実行前にコスト見積もり表示 |

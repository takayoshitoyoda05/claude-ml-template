...
| codex_gate.py | Stop | CLAUDE_CROSS_REVIEW=1 のとき Codexレビュー未完了ならブロック。センチネル(`.claude/checkpoints/codex_review_done.txt`)の HEAD ハッシュを現在の HEAD と照合し、レビュー後にコミットが進んでいたら再レビューを要求する |
...
これを現在の HEAD と照合する。HEAD が変わらない限りセンチネルは有効なままで、
再レビューは要求されない。**レビュー後にコミットを進める(HEAD が変わる)と
古いセンチネルは破棄され、ゲートが再び閉じる**(再度「クロスレビューして」で開く)。
つまり「すべての HEAD は一度 Codex レビューを通っている」ことを保証する仕組み。

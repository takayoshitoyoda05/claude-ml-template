...
| CLAUDE_ADVERSARIAL | `1` で敵対的レビュー(攻撃+偽陽性除外)を2軸レビュー後に実行 | 無効(0) |
...
9. `CLAUDE_ADVERSARIAL=1` なら **adversarial-reviewer(opus)** が差分を攻撃的に検証し、
   このセッション(リーダー)が指摘を1件ずつ懐疑的に再検証して偽陽性を除外する。
   実在する問題が残れば Generator に差し戻す(手順6.6)
...
| adversarial-reviewer | opus | 敵対的姿勢でコードの欠陥を暴く(第2層レビュー。CLAUDE_ADVERSARIAL=1で有効、指摘は検証パスで偽陽性除外) |
...
【第1層】evaluator(Spec) + evaluator-standards(Standards)
  ↓ 両方PASS
【第2層】adversarial-reviewer(Opus)が敵対的姿勢で攻撃
  → リーダーが指摘を1件ずつ懐疑的に再検証、偽陽性を除外
  → 実在する問題だけを差し戻し(Anthropic公式の Adversarial verification パターン)
  ↓ 実在する問題なし
【リファクタリング・パス】(磨き)
  ↓
...
第2層は友好的なレビューでは見えない層(密結合、暗黙の仮定、
繋がっていないエラーパス、評価指標を良く見せる方向のバグ)を検出する。
攻撃役の指摘は必ず検証パスを通り、再現経路を確認できた指摘だけが
差し戻しに使われる(偽陽性による無駄な差し戻しを防ぐ)。
...
第2層(Opus)と第3層(Fable)は高コストなモデルを使う。
毎回有効にするとパイプライン1回あたりのコストが体感2〜3倍になるため、
重要な変更(学習ロジック、評価指標、データ分割)のときだけ有効にする
運用も可能。両方ともデフォルトは無効(0)。
...
3. 通ったら段階的にゲートを足す: `CLAUDE_ENFORCE_EVAL=1` → 設計書+
   ...
   → 重要な変更のみ `CLAUDE_ADVERSARIAL` / `CLAUDE_FINAL_GATE`(3層レビュー。コスト注意は3.16節)
...
claude-ml-template/
  ...
  .claude/
    agents/
      ...
      adversarial-reviewer.md       Opus / 敵対的姿勢でコードの欠陥を暴く(第2層レビュー)

...
注意: `CLAUDE_WORK_SCOPE` を絶対パスで設定していると worktree 内への書き込みが
guard_scope にブロックされるため、その場合も逐次で実行される。
...
| 並列実装 | チームメイトの並列実行と worktree 分離の必要性は検証済み(2026-07-22)だが、**パイプライン1周での実走は未**。初回は逐次で通し、慣れてから試す |

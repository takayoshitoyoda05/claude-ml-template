...
| CLAUDE_ACTION_LOG | `0` でツール実行・エージェントの自動記録を無効化 | 有効(1) |
...
| report_gen.py | (手順8.5から実行) | evidence/ の機械集約(diff・ログ・テスト全出力・統計) |
...
パイプラインの全行動を機械的に記録し、完了時に2層レポートを必ず生成する。
モデルの記憶や善意に依存せず、フックとスクリプトが記録を保証する。

...
| L3 | 自律運転 | マージ確認のみ(計画は plan-reviewer が自動承認) |

レベルを解決するのは ml-pipeline のプロンプトで、フックは
CLAUDE_CONTROL_LEVEL を読まない。フックが直接読む変数(通知の
CLAUDE_NOTIFY 等)はレベルでは切り替わらないので、個別に設定する。

...
- **すること**: planner の検討経緯を一切知らない初見の立場で、前提の誤り・手順の順序や依存の抜け・
  検証方法の非検出性・影響範囲の見落とし・既存契約との衝突・不変条件と代理実装・
  並列グループの競合・誤委譲・ロールバック不能性の9観点を検証する。
...
brainstorm(発散)
  ↓ 方向を1つ選ぶ
design-interview(収束)
  - 曖昧性タクソノミー(境界値/同一性・解決/例外系/状態・順序/データ/非機能/
    スコープ境界/既存契約 + 研究系: 仮説/変数/ベースライン/成功基準/再現性)で聞き尽くす
...
#### 実装委譲(codex-delegate / CLAUDE_CODEX_IMPL)
...
さらに `CLAUDE_CODEX_IMPL=1` にすると、/ml-pipeline の**実装の既定主体が Codex になる**
(手順5「実装主体の選択」)。planner が「設計判断を含む」と印を付けたステップ/グループ
だけを Claude generator が担当し、残りは `codex exec -m $CODEX_IMPL_MODEL`
(実装用の安価なモデル。レビュー用モデルとは別指定)へ委譲される。単発実行で対話
できないため、依頼文は自己完結・不明点は実装せず質問出力、と指示し、コミット・
状態更新・対応表などの規律はリーダーが代行する。修正指示2回で解消しなければ
Claude generator が引き取る。実装主体別の差し戻し率をレポートに記録し、
委譲範囲の拡縮を判断する(効果測定つき導入)。
...
CLAUDE_CROSS_REVIEW=1 のとき、Codex は実装 diff のレビュー(手順5.5)に加えて
**計画・設計書のレビュー**(手順3.4、plan-premortem と並行の第2意見)にも使われる。
ドキュメントレビューではセンチネルを更新しない(センチネルはコード diff の
レビュー記録のため)。どちらも送信前に `scripts/data_scan.py` の検疫を通す。

#### 多モデル合議の規約(指摘一致の優先度ルール)

複数モデルの指摘は段階を問わず同じ規約で統合する(ml-pipeline「多モデル合議の規約」):
(1) 独立した2モデルが同じ箇所・同種の問題を指摘したら重大度を1段引き上げる
(独立の一致は問題実在の強い証拠)。(2) 単独の指摘は昇格も棄却もせず接地検証にかける —
**多数決はしない**。最終判定は常に接地(実測)であり、モデルの数ではない。
(3) 同じ file:line・同じコマンドについて主張が正面から矛盾したら HUMAN_REVIEW に回す。
